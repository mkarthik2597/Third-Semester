...
void MemAllocate(graph* g_ptr)
{
  ...
  g_ptr->root=malloc(n*sizeof(int));
...
void Kruskal(graph* g_ptr,edge** edgA, int E)
{
   ...
   for(i=0;i<g_ptr->n-1;i++)
   {
      ...
      if(g_ptr->root[v1-1]!=g_ptr->root[v2-1])
      {
        edgA[i]->selected=1;
        Union(g_ptr,v1,v2);
      }
...
{
  int i;
  if(g_ptr->TreeSize[g_ptr->root[v1-1]-1] > g_ptr->TreeSize[g_ptr->root[v2-1]-1])
  {
    for(i=0;i<g_ptr->n;i++)
    {
      if(g_ptr->root[i]==g_ptr->root[v2-1])
      {
        g_ptr->root[i]=g_ptr->root[v1-1];
        g_ptr->TreeSize[v1-1]++;
      }
    }
  }
  else
  {
    for(i=0;i<g_ptr->n;i++)
    {
      if(g_ptr->root[i]==g_ptr->root[v1-1])
      {
        g_ptr->root[i]=g_ptr->root[v2-1];
        g_ptr->TreeSize[v2-1]++;
      }
    }
  }
